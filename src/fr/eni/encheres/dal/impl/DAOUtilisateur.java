...
public class DAOUtilisateur implements Iutilisateur {
	...
	//Compte Modification 
	private static final String MODIFICATION_PROFIL = "UPDATE UTILISATEURS SET pseudo = ?, nom = ?, prenom = ?, email = ?, telephone = ?, rue = ?, code_postal = ?, ville = ?, mot_de_passe = ? WHERE no_utilisateur = ?;";
	...
	public void modificationProfil(Utilisateur utilisateur) {
		...
		try (Connection cnx = ConnectionProvider.getConnection()) {
			PreparedStatement pstmt = cnx.prepareStatement(MODIFICATION_PROFIL, PreparedStatement.RETURN_GENERATED_KEYS);
			pstmt.setString(1, utilisateur.getPseudo());
			pstmt.setString(2, utilisateur.getNom());
			pstmt.setString(3, utilisateur.getPrenom());
			pstmt.setString(4, utilisateur.getEmail());
	...
	public Utilisateur verificationEmailModif(String email, int no_utilisateur) {
		...
		try (Connection cnx = ConnectionProvider.getConnection()) {
			...
			PreparedStatement pstmt = cnx.prepareStatement(VERIFICATION_EMAIL_MODIF);
			pstmt.setString(1, email);
